    ...
    <div>
      <div>
        <div>
          ...
          {extensionText && (
            <span className="extension-badge">{extensionText}</span>
          )}
        </div>
        <div>
          ...
          {chord.notes.map((note, index) => (
            ...
            >
              ...
              {!isInScale(note.note) && <span className="text-red-500">*</span>}
              {index < chord.notes.length - 1 ? ", " : ""}
            </span>
          ))}
          {chord.notes.find((note) => !isInScale(note.note)) && (
            <div className="text-sm text-red-500">* Not in scale</div>
          )}
        </div>
      </div>
    </div>
