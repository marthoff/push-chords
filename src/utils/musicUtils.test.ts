...
test("shift a note", () => {
  expect(shiftNote({ note: "C", octave: 4 }, 2)).toEqual({
    note: "D",
    octave: 4,
  });
  expect(shiftNote({ note: "C", octave: 4 }, 12)).toEqual({
    note: "C",
    octave: 5,
  });
  expect(shiftNote({ note: "C", octave: 4 }, -1)).toEqual({
    note: "B",
    octave: 3,
  });
  expect(shiftNote({ note: "C", octave: 4 }, -13)).toEqual({
    note: "B",
    octave: 2,
  });
  expect(shiftNote({ note: "A", octave: 4 }, 5)).toEqual({
    note: "D",
    octave: 5,
  });
});
...
test("build a chromatic grid", () => {
  ...
  const grid = buildGrid(chromaticScale, chord, refScale);
  expect(grid[0][0].note.note).toBe("C");
